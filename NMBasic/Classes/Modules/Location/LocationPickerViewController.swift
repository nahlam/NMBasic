...
open class LocationPickerViewController: BasicUIViewController, MKMapViewDelegate {
        ...
        if self.location == nil {
            ...
            BasicLocationManager.sharedInstance.updateBlock = {(status , cllocation) in
                ...
                if cllocation != nil {
                    ...
                    self.region =  MKCoordinateRegion.init(center: cllocation!.coordinate, span: MKCoordinateSpan.init(latitudeDelta: 200, longitudeDelta: 200))
                    self.manageMapView()
                }
    ...
    @objc private func manageMapView()   {
        ...
        self.locationNameLabel.text = "\(String.localized(key: BasicFrameworkManager.Strings.Location.LocationStringPrefix))\(locationString)"
    ...
    private func locationChanged(newLocation: CLLocation? = nil)  {
        ...
        if let  center = mapView?.centerCoordinate  {
            ...
            location?.reverseGeocodeLocation(handler: { (address) in
                ...
                self.locationNameLabel.text = "\(String.localized(key: BasicFrameworkManager.Strings.Location.LocationStringPrefix))\(self.locationString)"
            })
        }
    }
    ...
    override open func setInitalUIStrings() {
        ...
        self.title = String.localized(key: BasicFrameworkManager.Strings.Location.LocationPickerTitle)
        self.locationHintLabel?.text = String.localized(key: BasicFrameworkManager.Strings.Location.LocationChooseHint)
    }
}
